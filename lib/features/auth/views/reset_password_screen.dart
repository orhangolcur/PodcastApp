...
class _ResetPasswordScreenState extends State<ResetPasswordScreen> {
  ...
  Future<void> _resetPassword() async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to reset. Check your code.'), backgroundColor: Colors.red),
      );
    }
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildTextField("OTP Code", _codeController, icon: Icons.vpn_key),
            SizedBox(height: 20.h),
            _buildTextField("New Password", _passwordController, icon: Icons.lock, isPassword: true),
            SizedBox(height: 20.h),
            _buildTextField("Confirm Password", _confirmController, icon: Icons.lock_outline, isPassword: true),
  ...
  Widget _buildTextField(String hint, TextEditingController controller, {required IconData icon, bool isPassword = false}) {
    return TextField(
